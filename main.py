...
def handle_customer(connection: duckdb.DuckDBPyConnection):
    table_name = "customer"
...
if __name__ == "__main__":
    with duckdb.connect("olist.db") as conn:
        handle_customer(conn)
        handle_geolocation(conn)
        handle_order_items(conn)
        handle_order_payments(conn)
        handle_order_reviews(conn)
        handle_orders(conn)
        handle_products(conn)
        handle_sellers(conn)
        handle_product_category_name_translation(conn)